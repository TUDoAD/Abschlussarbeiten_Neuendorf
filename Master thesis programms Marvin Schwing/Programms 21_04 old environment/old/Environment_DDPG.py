...
def ddpg(env_fn,
         ...
         mu_lr=1e-3,
         q_lr=1e-3,
         batch_size=200, #before 32 change to 200 15.2.22
         start_steps=2000, 
         action_noise=[5,1],
    ...
    def get_action(new_s, noise_scale):
        ...
        #print(a[0])
        return np.array([np.clip(a[0][0], -action_max[0], action_max[0]),np.clip(a[0][1], -action_max[1], action_max[1])])
    ...
    while stop == False:
        ...
        while not (d or (episode_length == max_episode_length)):
            ...
            if num_steps > start_steps:
                a = get_action(new_s, action_noise)
            else:
            ...
            replay_buffer.push(new_s, a, r, new_s2, d_store)
            inbuffer +=1
