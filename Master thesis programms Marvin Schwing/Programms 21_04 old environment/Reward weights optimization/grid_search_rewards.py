# -*- coding: utf-8 -*-
"""
Created on Fri Apr 22 16:24:23 2022
...
def Offline_Training(episodes, discount, learning_rate, env):
    ...
    def test_agent(extr_df):
        ...
        for j in range(num_episodes):
            ...
            #Take deterministic action at test time (noise_scale=0)
                ...
                state = extr_df.index[(extr_df[0]==obs_round[0])&(extr_df[1]==obs_round[1])&(extr_df[2]==obs_round[2])&(extr_df[3]==obs_round[3])].tolist()
    ...
    while stop == False:
        ...
        while done != True:
                ...
                new_state = extr_df.index[(extr_df[0]==obs2_round[0])&(extr_df[1]==obs2_round[1])&(extr_df[2]==obs2_round[2])&(extr_df[3]==obs2_round[3])].tolist()
                if new_state == []:
...
for reward_weights in [[1,0.3,0.1,0.1],[1,0.3,0.3,0.1],[1,0.3,0.5,0.1],[1,0.3,0.1,0.3],[1,0.3,0.3,0.3],[1,0.3,0.5,0.3],[1,0.3,0.1,0.5],[1,0.3,0.3,0.5],[1,0.3,0.5,0.5]]:
    own_env_dis = dis_env.ExtractionEnv_dis(wanted_drop_size, theta, excel_data, reward_weights,rpm_scale)
    test_env = dis_env.ExtractionEnv_dis(wanted_drop_size, theta, excel_data, reward_weights,rpm_scale)
    reward_plot, steps_plot, extr_df =  Offline_Training(5000, 0.99, 0.99, own_env_dis)
    with open(r'C:/Users/marvi/sciebo/Masterarbeit/Model/Q_table_old_env'+ str(reward_weights) + datetime.datetime.today().strftime('%Y_%m_%d %H_%M')+'.pkl', 'wb') as f:
        pickle.dump(extr_df, f)
#%%
print(str([1,0.3,0.1,0.1]))
print([1,0.3,0.1,0.1])