...
class Agent:
    ...
    def save_actor(self):
        self.actor.save(r'C:/Users/marvi/sciebo/Masterarbeit/Model/PPO old env'+ datetime.datetime.today().strftime('%Y_%m_%d %H_%M')+'.h5')
    ...
    def choose_action(self, observation):
        ...
        #print(prob[0])
        #dist = tfp.distributions.Categorical(probs=prob[0])
        #action = dist.sample()
        action = tf.squeeze(tf.random.categorical(prob, 1),axis=1)
        logprobability_all = tf.nn.log_softmax(prob)
        #print('action: {}'.format(action))
        #log_prob = dist.log_prob(action)
        log_prob = tf.reduce_sum(tf.one_hot(action,16)*logprobability_all, axis = 1)
        #print('log prob: {}'.format(log_prob))
        value = self.critic(state)
        return action.numpy()[0], value.numpy(), log_prob.numpy()
        ...
        for _ in range(self.n_epochs):
            ...
            for batch in batches:
                with tf.GradientTape(persistent=True) as tape:
                    ...
                    logprobability_all = tf.nn.log_softmax(prob)
                    new_probs = tf.reduce_sum(tf.one_hot(actions,16)*logprobability_all, axis = 1)
                    ...
                    min_advantage = tf.where(advantage[batch]>0,(1+self.policy_clip)*advantage[batch],(1-self.policy_clip)*advantage[batch])
                    
                    #weighted_probs = advantage[batch] * prob_ratio
                    #clipped_probs = tf.clip_by_value(prob_ratio, 1-self.policy_clip, 1+self.policy_clip)
                    #weighted_clipped_probs = clipped_probs*advantage[batch]
                    actor_loss = tf.minimum(prob_ratio*advantage[batch], min_advantage)#help_matrix = weighted_probs
                    print(actor_loss)
...
rpm_scale = 10
own_env = dis_env.ExtractionEnv_dis(wanted_drop_size, theta, excel_data, reward_weights,rpm_scale)
...
if __name__ == '__main__':
    ...
    batch_size = 128
    n_epochs = 1
    agent = Agent(0.9,0.2, 0.2, n_epochs, 0.99, batch_size,(4),(500,200,100,16), (500,200,100,1), 'softmax',None, 0.0003, 0.0003)
    test_agent_every = 25
    learn_iters = 0
    avg_score = 0 
    n_steps = 0
    ...
    while stop == False: 
        ...
        while not done:
            ...
            if info==True:
                ...
                store_d = done
            agent.store_transition(norm_obs_2,action,prob,val,new_val,reward,store_d) #change 25.04
            obs = copy.copy(obs_2)
...
dist = tfp.distributions.Categorical(probs=[0.1,0.1,0.8])
print(dist)
action = dist.sample()
print(action)
#%%
dist = tfp.distributions.Categorical(probs=[[0.8, 0.1, 0.1]])
n = 10
action = dist.sample()
print(action)
