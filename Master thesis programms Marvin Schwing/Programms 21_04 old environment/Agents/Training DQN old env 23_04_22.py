...
class DQNAgent:
    ...
    def save_model(self):
        self.model.save(r'C:/Users/marvi/sciebo/Masterarbeit/Model/DQN_old_env'+ datetime.datetime.today().strftime('%Y_%m_%d %H_%M')+'.h5')
...
if __name__ == '__main__':
    env = dis_env.ExtractionEnv_dis(wanted_drop_size, theta, excel_data, reward_weights,10) 
    test_env = dis_env.ExtractionEnv_dis(wanted_drop_size, theta, excel_data, reward_weights,10)
    returns = []
    steps = []
    agent = DQNAgent(env)
    state = env.reset()
